...
#define GPCR_LS_GOD 0

#define GPCR_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("GpcrRoutingProtocol");
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  {
    static TypeId tid = TypeId ("ns3::gpcr::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
/// Random number between [(-GPCR_MAXJITTER)-GPCR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (UniformRandomVariable ().GetValue (-GPCR_MAXJITTER, GPCR_MAXJITTER)))
#define FIRST_JITTER (Seconds(UniformRandomVariable().GetValue(0, GPCR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4
*/

/// Maximum allowed jitter.
#define GPCR_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-GPCR_MAXJITTER)-GPCR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (var->GetValue (-GPCR_MAXJITTER, GPCR_MAXJITTER)))
#define FIRST_JITTER (Seconds(var->GetValue(0, GPCR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4


NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for GPCR control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::GPCR_PORT = 666;
...
{
  static TypeId tid = TypeId ("ns3::gpcr::RoutingProtocol")
    ...
    .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                   EnumValue (GPCR_LS_GOD),
                   MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                   MakeEnumChecker (GPCR_LS_GOD, "GOD",
                                    GPCR_LS_RLS, "RLS"))
...
NS_LOG_FUNCTION (this << p->GetUid () << header.GetDestination () << idev->GetAddress ());
  if (m_socketAddresses.empty ())
    {
      NS_LOG_LOGIC ("No gpcr interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  NS_ASSERT (p != 0);
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      if (!tHeader.IsValid ())
        {
          NS_LOG_DEBUG ("GPCR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
          return false;
        }
      
      if (tHeader.Get () == GPCRTYPE_POS)
        {
          PositionHeader phdr;
        ...
        while(m_queue.Dequeue (dst, queueEntry))
          {
            ...
            if (!tHeader.IsValid ())
              {
                NS_LOG_DEBUG ("GPCR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
                return false;     // drop
              }
            if (tHeader.Get () == GPCRTYPE_POS)
              {
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
  ...
  if (l3->GetNAddresses (interface) > 1)
    {
      NS_LOG_WARN ("GPCR does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvGPCR, this));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), GPCR_PORT));//error order change 1
  socket->BindToNetDevice (l3->GetNetDevice (interface));//error order change 1
...
void
RoutingProtocol::RecvGPCR (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Address sourceAddress;
  Ptr<Packet> packet = socket->RecvFrom (sourceAddress);

  TypeHeader tHeader (GPCRTYPE_HELLO);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("GPCR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses ((interface) == 1))
      ...
      if (!socket)
        {
          ...
          socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), GPCR_PORT));
          socket->SetAllowBroadcast (true);
          m_socketAddresses.insert (std::make_pair (socket, iface));
    ...
    {
      NS_LOG_LOGIC ("GPCR does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in GPCR operation");
    }
}
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      socket->SendTo (packet, 0, InetSocketAddress (destination, GPCR_PORT));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid GPCR source address not found");
